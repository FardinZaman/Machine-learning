...
class ClickbaitDataset(Dataset):
  ...
  def __getitem__(self, idx):
    input_ids = self.tokenizer.encode(self.dataset[idx]['title_n_text'], truncation=True, max_length=self.max_input_len, return_tensors='pt')[0]
...
class ClickbaitDetector(pl.LightningModule):
  ...
  def training_step(self, batch, batch_nb):
    ...
    outputs, loss = self.forward(input_ids, labels)
  ...
  def validation_step(self, batch, batch_nb):
    ...
    input_ids, labels = batch
    outputs, vloss = self.forward(input_ids, labels)
    # vloss = nn.BCEWithLogitsLoss()(outputs, torch.tensor(labels, device=self.device, dtype=float).view(-1, 1))
  ...
  def validation_epoch_end(self, outputs):
    ...
    print(logs)
    print('pred', (val_pred.flatten() == 1).sum())
    print('ref', (val_ref.flatten() == 1).sum())
...
def main(args):
    ...
    def preprocess(example):
        title = '' if example['title'] == None else example['title']
        text = '' if example['text'] == None else example['text']
        example['title_n_text'] = title + '-' + text
        example['label'] = 0 if example['label'] == 'news' else 1
        return example

    dataset = load_dataset('csv', data_files=['train.csv', 'valid.csv']).pop('train')
    processed_dataset = dataset.filter(lambda example: example['label'] == 'news' or example['label'] == 'clickbait')
    processed_dataset = processed_dataset.map(preprocess)
    processed_dataset = processed_dataset.train_test_split(train_size=0.7, seed=817)
    clickbait_dataset = processed_dataset['test'].train_test_split(train_size=0.7, seed=817)
    clickbait_dataset['validation'] = clickbait_dataset.pop('train')
    clickbait_dataset['train'] = processed_dataset.pop('train')

    for split in ['train', 'validation', 'test']:
      ones = zeros = 0
      for i in range(clickbait_dataset[split].num_rows):
        if clickbait_dataset[split][i]['label'] == 0:
          zeros += 1
        elif clickbait_dataset[split][i]['label'] == 1:
          ones += 1
      print(f"In {split}: Ones: {ones}, Zeros: {zeros}")
    
    model.dataset = clickbait_dataset
